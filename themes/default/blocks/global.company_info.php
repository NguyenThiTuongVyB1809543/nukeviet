...
if (!nv_function_exists('nv_company_info')) {
    ...
    function nv_company_info_config($module, $data_block, $lang_block)
    {
        ...
        $html .= '<div class="form-group">';
        $html .= '<label class="control-label col-sm-6">' . $lang_global['company_contactphone'] . ':</label>';
        
        global $db, $nv_Lang; 
        $sql = "SELECT phone FROM " . NV_PREFIXLANG . "_contact_department";
        $result = $db->query($sql); 
        $phones = array();
        while ($row = $result->fetch()) {
            $phones[] = $row['phone'];
            // $html .= '<div class="col-sm-18"><input type="text" class="form-control" name="config_company_website" value="' . $row['phone'] . '"><span>' . $lang_global['multi_note'] . '</span></div>';
        } 
        $html .= '<div class="col-sm-18"><input type="text" class="form-control" name="config_company_website" value="' . implode(", ", $phones) . '"><span>' . $lang_global['multi_note'] . '</span></div>';
